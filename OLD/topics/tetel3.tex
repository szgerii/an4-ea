...
\subsection{Szeparábilis differenciálegyenlet megoldás integrálfüggvénnyel}
...
Bizonyítás a tételhez megtalálható a \ref{appendix:b} függelékben (ld. \ref{sec:egz-uni-proof}).
...
Véve a $\varphi'(x) = g(x)\cdot h(\varphi(x))$ feladatot, az
\vspace{-10pt}
...
Kicsit részletesebb levezetés megtalálható a \ref{appendix:b} függelékben (ld. \ref{sec:sep-exact-transform}).
...
Tehát ekkor a $\partial_2 g = \partial_1 h$ tekinthető \emph{szükséges feltételként} ahhoz, hogy egzakt differenciálegyenlet legyen a megadott feladat.
