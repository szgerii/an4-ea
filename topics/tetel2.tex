...
Az \ref{sec:impl-example} példában látott változók közötti "összefüggést" szeretnénk általánosítani.
...
Talán kicsit könnyebben feldolgozható a mátrix felépítése és a dimenziók összeadódása a következő, kibontott alakból:
...
Ezt szeretnénk általánosítani több változó esetére.
...
Megjegyezzük, hogy a \ref{eq:loc-inv-starter} ekvivalencia alapján egy tetszőleges $y \in \R^n$ vektorra pontosan akkor fog teljesülni $f(x) = y$, ha $F_y(x) = x$. Innen jön az ötlet, hogy induljunk el az $F_y(x)$-en vett fixpont-tétel felé, ami majd megadja ezt az $x$-et.
}
...
A \ref{eq:loc-inv-F-approx} egyenlőtlenségben kapott becsléssel azt is beláttuk, hogy $F_y$ egy kontrakció, $q = 0.5$ kontrakciós együtthatóval. Továbbá $K_r(0) \subset \overline{K_r(0)}$ és a \ref{eq:loc-inv-Fy-rng}-ben tett megállapításunk alapján alkalmazhatjuk $F_y$-ra a \emph{fixpont-tételt}.
...
Ez alapján definiáljunk egy $\varphi : K_{r/2}(0) \to \overline{K_r(0)}$ leképezést, amire $\varphi(y) := x_y \enspace (y \in K_{r/2}(0))$\footnote{
Itt van egy minimális eltérés az előadástól, ott ugyanis pár állítást a következőkből még $x_y$-al fogalmazunk meg, aztán vezetjük csak be a $\varphi$ függvényt (valamint ott $\varphi$ helyett $\varrho$-val jelöltük ezt a függvényt). A két módszer természetesen ekvivalens, és a segédanyagban is az itt látható azonnali áttérés található.
}.
...
Felhasználva a \ref{eq:loc-inv-Fy-rng} korlátunkat, megállapítjuk, hogy
\begin{equation*}
...
Továbbá mivel $h$ egy kontrakció (ld. \ref{eq:loc-inv-F-approx} korlát),
\begin{gather*}
